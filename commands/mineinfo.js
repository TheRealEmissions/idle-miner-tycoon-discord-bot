        ...
        if (args[1]) {
            ...
            if (client.mineSelected.has(message.author.id)) {
                client.models.userProfiles.findOne({
                    ...
                    const mineType = docs.mines.find(x => x.index == client.mineSelected.get(message.author.id)).type;
                    const mine = client.storage.mines.find(x => x.type.toUpperCase() == mineType);
                    message.channel.send(new client.modules.Discord.MessageEmbed()
                        .setColor(message.guild.me.displayHexColor)
                        .addField(`**${mine.type} Mine**`, `\`Rarity\` ${client.functions.noToStarEmoji(mine.rarity)}\n\`Sell Price/KG\` $${client.functions.formatNumber(mine.ppk)}\n\`KG/s\` ${mine.kgps}`)
                    );
