        ...
        client.models.userCrates.findOne({
            ...
            let embed = {
                embed: {
                    ...
                    title: `**${message.author.username}${message.author.username.endsWith('s') ? `'` : `'s`} Crates:**`,
                    description: `** **\n:one: **Open a crate**\n:x: **Close this menu**\n**⠀**`,
                    fields: [{
                        name: 'Mine Crates',
                        value: minecrates
            ...
            message.channel.send(embed).then(msg => {
                msg.react(`1⃣`);
                msg.react(`❌`);
                let collector = new client.modules.Discord.ReactionCollector(msg, (reaction, user) => ((reaction.emoji.name == `1⃣`) || (reaction.emoji.name == '❌')) && user.id == message.author.id, {
                    max: 1
                });
                collector.on('collect', reaction => {
                    ...
                    if (reaction.emoji.name == `1⃣`) {
                        ...
                        collector.on('collect', id => {
                            collector.stop();
                            id.delete();
                            new client.methods.crateHandler(client, message, msg, this.crates.get(Number(id.content)));
                        });
                    }
