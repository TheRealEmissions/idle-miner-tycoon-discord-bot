...
module.exports = class crateHandler {
    ...
    getMineCrateDrop(star) {
        return new Promise((resolve, reject) => {
            let drop = this.client.storage.mines[this.client.functions.genNumberBetween(0, this.client.storage.mines.length - 1)];
            while (drop.rarity > star) {
                drop = this.client.storage.mines[this.client.functions.genNumberBetween(0, this.client.storage.mines.length - 1)];
            }
            return resolve(drop);
        });
    }
    ...
    async openMineCrate(star) {
        this.msg.edit(new this.client.modules.Discord.MessageEmbed()
            .setColor(this.message.guild.me.displayHexColor)
            .setTitle(`**Opening a ${this.client.functions.noToStarEmoji(star)} Mine Crate:**`)
            .setDescription(`** **\n**Please select a crate from below**`)
        )
        let emojis = ["632981632273350656", "632983812023910430", "632983891313164289", "632983923286343698", "632983991833722902"];
